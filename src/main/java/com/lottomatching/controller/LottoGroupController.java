...
public class LottoGroupController {
    ...
    @RequestMapping(value = "/admin/lottoGroup/{barcode}", method = RequestMethod.GET)
    public ModelAndView LottoGroupLottoDetail(@PathVariable("barcode") String barcode) {
        ModelAndView modelAndView = new ModelAndView();
        ...
        Lotto lottoDetail = lottoService.findByBarcode(barcode);

        modelAndView.addObject("lottoDetail", lottoDetail);
        modelAndView.addObject("s2", s2);
        modelAndView.addObject("s3", s3);
        modelAndView.addObject("s4", s4);
        modelAndView.addObject("s5", s5);
